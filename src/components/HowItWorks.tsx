          ...
          {steps.map((step, index) => (
            <div key={index} className="relative">
              ...
              {index < steps.length - 1 && (
                <img
                  src="/Union.png" // <-- your connector image
                  alt="connector"
                  className="hidden lg:block absolute top-16 left-full -translate-x-1/2 z-0"
                />
              )}

              <div className="bg-white rounded-2xl p-6 shadow flex flex-col h-full relative z-10">
                {/* Step Number */}
                <div className="text-center text-[112px] font-bold text-[#2D96BC] mb-4 leading-none">
                  {step.number}
                </div>

                {/* Title */}
                <h3 className="text-[24px] font-semibold text-center mb-2">
                  {step.title}
                </h3>

                {/* Description - flex-grow pushes image to bottom */}
                <p className="text-muted-foreground text-[16px] text-center leading-relaxed flex-grow">
                  {step.description}
                </p>

                {/* Image always bottom aligned */}
                <img
                  src={step.image}
                  alt={step.title}
                  className="mx-auto mt-6 w-auto h-[150px]"
                />
              </div>
            </div>
          ))}
